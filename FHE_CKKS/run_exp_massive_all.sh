...
BINARY=./build/bin/run_he_inference_massive
N_THREADS=32

mkdir -p logs

###############################################################
# Phase 1: Sanity test on MASSIVE full-train (200 samples)
###############################################################
echo
echo "=== Phase 1: SANITY (MASSIVE, 200 samples) ==="
"${BINARY}" he_artifacts_qwen3_massive_intent_h256 200 8 40 \
    "60,40,40,40,40,40,60" "massive_sanity" 16384 \
    2>&1 | tee logs/run_massive_sanity.log
...
###############################################################
# Phase 2: Exp 2 — precision sweep on MASSIVE full-train
###############################################################
echo
echo "=== Phase 2: Exp 2 — Precision sweep (MASSIVE) ==="
...
for cfg in "${EXP2_CONFIGS[@]}"; do
    ...
    BITS="${cfg##*:}"
    TAG="s${SCALE}"
    LOG="logs/run_massive_${TAG}.log"
    echo "  scale=2^${SCALE}  bits=${BITS}  tag=${TAG}"
    "${BINARY}" he_artifacts_qwen3_massive_intent_h256 0 "${N_THREADS}" \
        "${SCALE}" "${BITS}" "${TAG}" 16384 \
        2>&1 | tee "${LOG}" || echo "[!] ${TAG} failed"
done

###############################################################
# Phase 3: Exp 3 — config feasibility on MASSIVE full-train
###############################################################
echo
echo "=== Phase 3: Exp 3 — Config feasibility (MASSIVE) ==="
...
###############################################################
# Phase 4: Exp 4 — data efficiency on MASSIVE small-pools
###############################################################
echo
echo "=== Phase 4: Exp 4 — Data efficiency (MASSIVE small-pools) ==="
declare -a POOLS=(
...
for ART in "${POOLS[@]}"; do
    ...
    TAG="${ART#he_artifacts_qwen3_massive_intent_}"
    LOG="logs/run_massive_${TAG}.log"
...
echo
echo "============================================"
echo "All MASSIVE experiments complete."
echo "============================================"