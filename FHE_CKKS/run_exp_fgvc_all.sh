...
BINARY=./build/bin/run_he_inference_fgvcaircraft
N_THREADS=32

###############################################################
# Phase 1: Sanity test on FGVC-Aircraft full-train (200 samples)
###############################################################
echo
echo "=== Phase 1: SANITY (FGVC, 200 samples) ==="
"${BINARY}" he_artifacts_dinov2_fgvcaircraft_h256 200 8 40 \
    "60,40,40,40,40,40,60" "fgvc_sanity" 16384 \
    2>&1 | tee logs/run_fgvc_sanity.log
echo "If above ran successfully, press Enter to continue with full experiments"
echo "(or Ctrl-C to abort)"
read

###############################################################
# Phase 2: Exp 2 — precision sweep on FGVC full-train
###############################################################
echo
echo "=== Phase 2: Exp 2 — Precision sweep (FGVC) ==="
...
for cfg in "${EXP2_CONFIGS[@]}"; do
    ...
    BITS="${cfg##*:}"
    TAG="s${SCALE}"
    LOG="logs/run_fgvc_${TAG}.log"
    echo "  scale=2^${SCALE}  bits=${BITS}  tag=${TAG}"
    "${BINARY}" he_artifacts_dinov2_fgvcaircraft_h256 0 "${N_THREADS}" \
        "${SCALE}" "${BITS}" "${TAG}" 16384 \
        2>&1 | tee "${LOG}" || echo "[!] ${TAG} failed"
done

###############################################################
# Phase 3: Exp 3 — config feasibility on FGVC full-train
###############################################################
echo
echo "=== Phase 3: Exp 3 — Config feasibility (FGVC) ==="
...
###############################################################
# Phase 4: Exp 4 — data efficiency on FGVC small-pools
###############################################################
echo
echo "=== Phase 4: Exp 4 — Data efficiency (FGVC small-pools) ==="
declare -a POOLS=(
...
for ART in "${POOLS[@]}"; do
    ...
    TAG="${ART#he_artifacts_dinov2_fgvcaircraft_}"
    LOG="logs/run_fgvc_${TAG}.log"
...
echo
echo "============================================"
echo "All FGVC experiments complete."
echo "============================================"