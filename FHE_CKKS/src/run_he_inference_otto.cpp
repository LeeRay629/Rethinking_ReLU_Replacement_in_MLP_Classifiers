...
public:
        ...
        size_t B, size_t hidden_dim, size_t n1, size_t n2)
    {
        ...
        for (size_t j = 1; j < n1; ++j)
            steps.push_back(static_cast<int>(j * B));
        for (size_t i = 1; i < n2; ++i)
            steps.push_back(static_cast<int>(i * n1 * B));
        for (size_t s = 1; s <= hidden_dim / 2; s *= 2)
            steps.push_back(static_cast<int>(s * B));
        ...
        n1_(n1), n2_(n2)
    {
        ...
        if (n1_ * n2_ < meta_.input_dim)
            throw runtime_error("BSGS factorization n1*n2 must be >= input_dim");
        if (meta_.hidden_dim % meta_.input_dim != 0)
            throw runtime_error("input_dim must divide hidden_dim "
                "(= slot_count/B) for BSGS cyclic correctness");
    ...
    Ciphertext encrypt_input_batched(const vector<vector<double>>& xs,
        double& batch_t_enc) const
    {
        auto t0 = clk::now();
        vector<double> packed = pack_batch_interleaved(
            xs, meta_.input_dim, B_, rt_.slot_count);
        Plaintext pt;
        rt_.encoder->encode(packed, rt_.scale, pt);
        Ciphertext ct;
        rt_.encryptor->encrypt(pt, ct);
        batch_t_enc = chrono::duration<double, milli>(clk::now() - t0).count();
        return ct;
    }

    Ciphertext layer1_matvec(const Ciphertext& ct_x, double& matvec_ms) const {
        auto& ev = *rt_.evaluator;
        auto t0 = clk::now();
        ...
        for (size_t j = 1; j < n1_; ++j)
            ev.rotate_vector(ct_x, static_cast<int>(j * B_),
                *rt_.galois_keys, baby[j]);
        ...
        for (size_t i = 0; i < n2_; ++i) {
            ...
            for (size_t j = 0; j < n1_; ++j) {
                size_t k = i * n1_ + j;
                if (k >= meta_.input_dim) break;
                Ciphertext term;
                ev.multiply_plain(baby[j], pt_diag_bsgs_[i][j], term);
                if (!inner_set) { inner = std::move(term); inner_set = true; }
                else { ev.add_inplace(inner, term); }
            }
            if (!inner_set) continue;
            if (i > 0)
                ev.rotate_vector_inplace(inner,
                    static_cast<int>(i * n1_ * B_), *rt_.galois_keys);
    ...
    vector<Ciphertext> output_layer_multiclass(const Ciphertext& ct_p,
        ...
        for (size_t c = 0; c < C; ++c) {
            ...
            for (size_t s = H / 2; s >= 1; s /= 2) {
                Ciphertext rot;
                ev.rotate_vector(ct_s, static_cast<int>(s * B_),
                    *rt_.galois_keys, rot);
...
private:
    void precompute_layer_plaintexts(const std::set<int>& post_act_levels) {
        ...
        auto L0 = chain.at(0);
        pt_diag_bsgs_.assign(n2_, std::vector<Plaintext>(n1_));
        for (size_t i = 0; i < n2_; ++i) {
            for (size_t j = 0; j < n1_; ++j) {
                size_t k = i * n1_ + j;
                vector<double> diag(rt_.slot_count, 0.0);
                if (k < d) {
                    diag = build_diagonal_batched(W1_, H, d, k, B_, rt_.slot_count);
                    if (i > 0) {
                        long long shift = -static_cast<long long>(i * n1_ * B_);
                        diag = plaintext_rotate(diag, shift, rt_.slot_count);
                    }
                }
                rt_.encoder->encode(diag, L0, rt_.scale, pt_diag_bsgs_[i][j]);
            }
        }
        ...
        cout << "[Engine] Pre-encoded W2 at " << pt_W2_cache_.size()
            << " level(s); diagonals: " << (n1_ * n2_) << " plaintexts.\n";
    }
    ...
    vector<double>       W2_;
    vector<double>       b2_;
    std::vector<std::vector<Plaintext>> pt_diag_bsgs_;
...
int main(int argc, char** argv) {
    ...
    // Pad input_dim
    const size_t cyclic_period = meta.hidden_dim;
    size_t input_dim_padded = orig_input_dim;
    while (input_dim_padded <= cyclic_period &&
        cyclic_period % input_dim_padded != 0) {
        ++input_dim_padded;
    }
    if (cyclic_period % input_dim_padded != 0)
    ...
    if (input_dim_padded != orig_input_dim) {
        cout << "[Pad] input_dim " << orig_input_dim << " -> " << input_dim_padded
            << " (so that hidden_dim=" << meta.hidden_dim
            << " is a multiple, BSGS cyclic-correct)\n";
    ...
    size_t n1 = static_cast<size_t>(std::ceil(std::sqrt(static_cast<double>(meta.input_dim))));
    while (n1 * ((meta.input_dim + n1 - 1) / n1) < meta.input_dim) ++n1;
    size_t n2 = (meta.input_dim + n1 - 1) / n1;
    cout << "Batch        : B=" << B << "   BSGS: n1=" << n1 << "  n2=" << n2
        << "   (input_dim_used=" << meta.input_dim << ")\n";

    auto rotation_steps = InferenceEngine::compute_rotation_steps(
        B, meta.hidden_dim, n1, n2);
    ...
    auto worker = [&](unsigned tid) {
            ...
            < n_batches)
        {
            ...
            Ciphertext ct_x = engine.encrypt_input_batched(xs, batch_t_enc);

            double batch_t_matvec = 0.0;
            Ciphertext ct_y = engine.layer1_matvec(ct_x, batch_t_matvec);
